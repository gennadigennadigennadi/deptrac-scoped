 ...
 */
namespace DEPTRAC_1700753084\Symfony\Component\Yaml;
    ...
    private function doParse(string $value, int $flags) : mixed
    {
        ...
        do {
            ...
            if ('-' === $this->currentLine[0] && self::preg_match('#^\\-((?P<leadspaces>\\s+)(?P<value>.+))?$#u', \rtrim($this->currentLine), $values)) {
                ...
                } elseif (!isset($values['value']) || '' == \trim($values['value'], ' ') || \str_starts_with(\ltrim($values['value'], ' '), '#')) {
                ...
                if (\str_starts_with($values['key'], '!php/const:')) {
                    \DEPTRAC_1700753084\trigger_deprecation('symfony/yaml', '6.2', 'YAML syntax for key "%s" is deprecated and replaced by "!php/const %s".', $values['key'], \substr($values['key'], 11));
                }
